...
public class MXEditor : Editor {

	[MenuItem("Assets/MX/创建字体")]  
	...
	/// </summary>
	/// <returns></returns>
	static List<Button> GetAllButtons() {
	...
	[MenuItem("Assets/MX/所有Button/添加ButtonPlus")]
	static void AllButtonsAddButtonPlus() {
		List<Button> list = GetAllButtons();
		foreach (Button item in list)
		{
			ButtonPlus buttonPlus = item.gameObject.GetComponent<ButtonPlus>();
			if (buttonPlus == null) {
				item.gameObject.AddComponent<ButtonPlus>();
			}
		}	
	}
	[MenuItem("Assets/MX/所有Button/移除ButtonPlus")]
	static void AllButtonsRemoveButtonPlus() {
		List<Button> list = GetAllButtons();
		foreach (Button item in list)
		{
			ButtonPlus buttonPlus = item.gameObject.GetComponent<ButtonPlus>();
			DestroyImmediate(buttonPlus, true);
		}
	}
}
